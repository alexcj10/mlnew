...
def show_packages_list():
    ...
    print(f"    {dim('vantix init myproject --pkg numpy==1.24.0 --pkg pandas==2.0.0')}")
    print(f"    {dim('vantix init myproject --pkg numpy==latest')}")
    print(f"    {dim('vantix init myproject --pkg torch --pkg transformers')}")
    print()
...
ML project scaffolded with [Vantix](https://pypi.org/project/vantix/).
...
def main():
    parser = argparse.ArgumentParser(
        description=bold("  Vantix — ML Project Scaffolding CLI"),
    ...
    if args.version:
        try:
            ...
            ver = pkg_version("vantix")
        except Exception:
            try:
                from vantix import __version__ as ver
            except ImportError:
                ver = "unknown"
        print(f"  Vantix version {ver}")
        return


    if args.help or not args.command:
        print()
        print(bold("  Vantix — ML Project Scaffolding CLI"))
        print()
        print("  Usage:")
        print(f"    {cyan('vantix init <project_name>')}              Create project with default packages")
        print(f"    {cyan('vantix init <project_name> --pkg <spec>')} Override specific package versions")
        print(f"    {cyan('vantix packages')}                         List default packages and versions")
        print(f"    {cyan('vantix --version')}                        Show version")
        print()
        print("  Examples:")
        print(f"    {dim('vantix init my_project')}")
        print(f"    {dim('vantix init my_project --pkg numpy==1.24.0 --pkg pandas==2.0.0')}")
        print(f"    {dim('vantix init my_project --pkg numpy==latest --pkg torch')}")
        print()
        return
    ...
    error(f"Unknown command: '{args[0]}'. Run 'vantix --help' for usage.")
